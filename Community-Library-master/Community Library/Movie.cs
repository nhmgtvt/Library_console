...
public class Movie : IMovie
{
    ...
    public bool AddBorrower(IMember member)
    {
        if (this.AvailableCopies <= 0)
            return false;
        int countNumberBeforeAdd = borrowers.Number;
        borrowers.Add(member);
        if (borrowers.Number > countNumberBeforeAdd)
        {
    ...
    {
        int countNumberBeforeDelete = borrowers.Number;
        borrowers.Delete(member);
        if (borrowers.Number < countNumberBeforeDelete)
        {
            this.AvailableCopies++;
            return true;
        }
    ...
    {
        // return this.TotalCopies.CompareTo(another.TotalCopies);
    ...
    {
        return string.Format("Movie title:{0} Genre:{1} Classification:{2} Duration:{3} Available copies:{4}", this.Title, this.Genre, this.Classification, this.Duration, this.AvailableCopies);
    }
}
